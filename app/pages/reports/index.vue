...
const hasValidPhone = computed(() => {
  return Boolean(normalizedPhone.value && normalizedPhone.value.length >= 10)
})
...
// Trigger fetch when courier changes
watch(selectedCourierId, (newId) => {
  if (newId) {
    fetchCourierReport()
  }
})
...
const handleSendWhatsApp = () => {
  ...
  if (!hasValidPhone.value) {
    toast.error('Bu kurye için kayıtlı geçerli bir telefon numarası bulunamadı. Lütfen kurye ayarlarına telefon numarası ekleyin.', 'Telefon Eksik')
    return
  }
...
<template>
  <div class="space-y-6">
    ...
    <div class="bg-white p-4 sm:p-5 rounded-xl border border-slate-200 shadow-xs space-y-4">
      ...
      <div class="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100 text-xs">
        <div class="flex flex-wrap items-center gap-1.5 text-slate-600">
          <span class="text-[11px] font-semibold text-slate-400 mr-1">Hızlı Tarih:</span>
          <button
            type="button"
            class="px-2.5 py-1 rounded-md border border-slate-200 hover:bg-slate-100 text-slate-700 font-medium transition-colors"
    ...
    <div v-if="!selectedCourierId" class="bg-white rounded-xl border border-dashed border-slate-300 p-12 text-center space-y-3">
      ...
      <h3 class="text-base font-semibold text-slate-800">Lütfen bir kurye seçin</h3>
    ...
    <!-- DURUM 2: Yükleniyor -->
    <div v-else-if="loading" class="bg-white rounded-xl border border-slate-200 p-12 text-center space-y-3">
      <RefreshCw class="w-8 h-8 text-emerald-600 animate-spin mx-auto" />
      <div class="text-sm font-semibold text-slate-700">Kurye hakediş raporu hesaplanıyor...</div>
      <div class="text-xs text-slate-400">Veritabanındaki anlık fiyat ve toplam tutarlar derleniyor.</div>
    </div>
    ...
    <div v-else-if="reportData" class="space-y-6">
      ...
      <div
        v-if="!hasValidPhone"
        class="p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3 text-amber-800 text-xs sm:text-sm"
      >
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <div class="text-lg font-bold text-slate-900 mt-1 flex items-center gap-2">
              <Bike class="w-5 h-5 text-emerald-600" />
              <span>{{ reportData.courier.name }}</span>
            </div>
            <div class="flex items-center gap-1.5 text-xs text-slate-500 mt-1 font-mono">
              <Phone class="w-3.5 h-3.5 text-slate-400" />
              <span>{{ reportData.courier.phone || 'Telefon Kayıtlı Değil' }}</span>
            </div>
          </div>

          <div class="pt-3 mt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-600">
            <span class="text-slate-400 font-medium">Tarih Aralığı:</span>
        ...
        <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between">
          ...
          <div class="pt-3 mt-3 border-t border-slate-100 grid grid-cols-2 gap-2 text-xs">
        ...
        <div class="bg-slate-950 p-5 rounded-xl border border-slate-800 shadow-sm text-white flex flex-col justify-between relative overflow-hidden">
      ...
      <div
        ...
        class="bg-white rounded-xl border border-slate-200 p-10 text-center space-y-2"
      >
        <div class="text-sm font-semibold text-slate-700">Bu tarih aralığında seçilen kurye için hakediş kaydı bulunamadı.</div>
      ...
      <div v-else class="space-y-6">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div class="p-4 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-slate-50/60">
            <div>
            ...
            <table class="w-full text-left border-collapse text-xs sm:text-sm">
              <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th class="px-4 py-3 font-semibold text-slate-700 text-center">Tür</th>
                  <th class="px-4 py-3 font-semibold text-slate-700 text-right">Paket</th>
                  <th class="px-4 py-3 font-semibold text-slate-700 text-right">Birim Fiyat</th>
                  <th class="px-4 py-3 font-semibold text-slate-900 text-right">Hakediş</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-100">
                ...
                >
                  ...
                  <!-- Hakediş (totalAmount) -->
                  <td class="px-4 py-3 text-right font-bold text-emerald-700 font-mono">
        ...
        <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4">
          ...
          <div class="bg-slate-50 p-4 sm:p-5 rounded-lg border border-slate-200 font-mono text-xs sm:text-sm text-slate-800 whitespace-pre-line leading-relaxed select-all">
    ...
    >
      <div class="space-y-4">
        <div v-if="!hasValidPhone" class="p-3 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-xs">
        ...
        <div class="bg-slate-900 text-slate-100 p-4 rounded-xl text-xs sm:text-sm font-mono whitespace-pre-line max-h-96 overflow-y-auto leading-relaxed border border-slate-800 select-all">
          {{ formattedInvoiceText }}
        </div>
