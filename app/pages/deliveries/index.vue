...
const onAddVenueChange = (newVenueId: string) => {
  ...
  if (newVenueId) {
    ...
    if (selected) {
      addForm.value.indoorPrice = selected.indoorPrice
      addForm.value.outdoorPrice = selected.outdoorPrice
    }
  }
}
...
<template>
  <div class="space-y-6">
    ...
    <div v-else class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <table class="w-full text-left border-collapse text-xs sm:text-sm">
          <thead class="bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700">
            <tr>
              ...
              <th class="px-4 py-3 font-semibold text-slate-700 dark:text-slate-300 text-right">Paket Sayısı</th>
              <th class="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100 text-right">Toplam Hakediş</th>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            ...
            >
              ...
              <td class="px-4 py-3.5 text-right font-mono text-slate-700 dark:text-slate-300 font-semibold">
              ...
              <td class="px-4 py-3.5 text-right font-bold text-slate-900 dark:text-slate-100 font-mono">
                <span class="inline-flex items-center px-2.5 py-1 rounded bg-emerald-50 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300 text-xs sm:text-sm font-bold border border-emerald-200/60 dark:border-emerald-800/60">
      ...
      <div class="px-4 py-3.5 bg-slate-50 dark:bg-slate-800/60 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-slate-600 dark:text-slate-400 font-medium">
        ...
        <div class="flex flex-wrap items-center gap-4 text-xs">
          <span class="text-emerald-800 dark:text-emerald-400 font-semibold">
            İç Mekan: <strong class="font-mono text-emerald-900 dark:text-emerald-300 font-bold">{{ (summary.indoorSettlementAmount || 0).toFixed(2) }} ₺</strong> ({{ summary.indoorPackages }} Paket)
          </span>
          <span class="text-slate-300 dark:text-slate-700 hidden sm:inline">|</span>
          <span class="text-sky-800 dark:text-sky-400 font-semibold">
            Dış Mekan: <strong class="font-mono text-sky-900 dark:text-sky-300 font-bold">{{ (summary.outdoorSettlementAmount || 0).toFixed(2) }} ₺</strong> ({{ summary.outdoorPackages }} Paket)
          </span>
          <span class="text-slate-300 dark:text-slate-700 hidden sm:inline">|</span>
          <span class="text-slate-900 dark:text-slate-100 font-bold">
            Genel Toplam: <strong class="text-emerald-700 dark:text-emerald-400 font-extrabold font-mono text-sm">{{ (summary.totalSettlementAmount || 0).toFixed(2) }} ₺</strong>
          </span>
        </div>
      </div>
    </div>
      ...
      <form class="space-y-4" @submit.prevent="handleAddSubmit">
        ...
        <div class="p-3.5 rounded-xl bg-emerald-50/70 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-800/60 space-y-2.5">
          <div class="flex items-center justify-between">
            ...
            <span v-if="indoorTotal > 0" class="text-xs font-bold font-mono text-emerald-800 dark:text-emerald-300 bg-white dark:bg-slate-800 px-2 py-0.5 rounded border border-emerald-300 dark:border-emerald-700">
        ...
        <div class="p-3.5 rounded-xl bg-sky-50/70 dark:bg-sky-950/40 border border-sky-200/80 dark:border-sky-800/60 space-y-2.5">
          <div class="flex items-center justify-between">
            ...
            <span v-if="outdoorTotal > 0" class="text-xs font-bold font-mono text-sky-800 dark:text-sky-300 bg-white dark:bg-slate-800 px-2 py-0.5 rounded border border-sky-300 dark:border-sky-700">
    ...
    <BaseModal
      ...
      description="Paket kaydı bilgilerini, mekanını, birim fiyatını ve paket adedini güncelleyin."
    >
      <form class="space-y-4" @submit.prevent="handleEditSubmit">
        ...
        <div class="p-3.5 rounded-xl bg-slate-900 dark:bg-slate-800 text-white space-y-2 border border-slate-800 dark:border-slate-700">
          ...
          <div class="flex items-center justify-between text-sm font-bold text-white border-t border-slate-800 dark:border-slate-700 pt-2">
